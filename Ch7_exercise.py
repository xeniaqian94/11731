...
def train(args):
    ...
    print training_src[0], training_src_id[0]
    ...
    print training_tgt[0], training_tgt_id[0]

    dev_src = read_corpus(args.dev_src)  # get vocabulary
    dev_src_id = src_vocab.get_data_id(dev_src)
    print dev_src[0], dev_src_id[0]

    dev_tgt = read_corpus(args.dev_tgt)  # get vocabulary
    dev_tgt_id = tgt_vocab.get_data_id(dev_tgt)
    print dev_tgt[0], dev_tgt_id[0]
    ...
    for epoch in range(epochs):
        for (src_batch, tgt_batch) in get_batches(train_pair, args.batch_size):
            ...
            if updates % eval_every == 0:
                bleu_score, translation = translate(model, dev_pair, src_vocab.i2w, tgt_vocab.i2w)
...
class EncoderDecoder:
    ...
    def translate_sentences(self, sent, max_len=50):
        dy.renew_cg()
        W_y = dy.parameter(self.W_y)
        b_y = dy.parameter(self.b_y)
        ...
        enc_state = self.enc_builder.initial_state()
        for cw in sent:
            dy.lookup(self.src_lookup, int(cw))
            enc_state = enc_state.add_input(x_t)
        encoded = enc_state.output()
        ...
        while len(trans_sentence) < max_len:
            ...
            x_t = dy.lookup(self.tgt_lookup, self.tgt_token_to_id[cw])
            dec_state = dec_state.add_input(x_t)
            ...
            cw = self.tgt_id_to_token[np.argmax(p)]
            if cw == '</S>':
                break
